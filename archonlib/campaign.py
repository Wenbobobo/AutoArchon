    ...
    *,
    ...
    return "\n".join(
        [
            ...
            "}",
            "",
            "helper_default_api_key_env() {",
            '  case "${ARCHON_HELPER_PROVIDER:-openai}" in',
            '    gemini) printf "%s\\n" "GEMINI_API_KEY" ;;',
            '    openrouter) printf "%s\\n" "OPENROUTER_API_KEY" ;;',
            '    *) printf "%s\\n" "OPENAI_API_KEY" ;;',
            "  esac",
            ...
            "}",
            "",
            "normalize_helper_binding() {",
            '  local binding_var="$1"',
            '  local default_env="$2"',
            '  local raw_value="${!binding_var:-}"',
            '  if [[ -z "${raw_value}" ]] || is_helper_env_name "${raw_value}"; then',
            "    return 0",
            "  fi",
            '  printf -v "${default_env}" "%s" "${raw_value}"',
            '  export "${default_env}"',
            '  printf -v "${binding_var}" "%s" "${default_env}"',
            '  export "${binding_var}"',
            "}",
            "",
                ...
                running_rows.append(
                    {
                        ...
                        "helperFailedCallCount": run.get("helperFailedCallCount"),
                        "helperFailedReasonCounts": run.get("helperFailedReasonCounts"),
                        ...
                        "acceptedFormalizationCount": (
                            len(run.get("acceptedFormalizations", []))
                            if isinstance(run.get("acceptedFormalizations"), list)
                            else 0
                        ),
...
def create_campaign(
    *,
    ...
    if campaign_root.exists():
        ...
        if any(path.name != "control" for path in existing):
...
def collect_campaign_status(campaign_root: Path, *, heartbeat_seconds: int = DEFAULT_HEARTBEAT_SECONDS) -> dict[str, Any]:
    ...
    for run in runs:
        ...
        run_summary = {
            ...
            "helperFailedCallCount": (
                int(supervisor_progress.get("helper", {}).get("failedCallCount", 0))
                if isinstance(supervisor_progress, Mapping)
                and isinstance(supervisor_progress.get("helper"), Mapping)
                else 0
            ),
            "helperFailedReasonCounts": (
                dict(supervisor_progress.get("helper", {}).get("failedCallsByReason", {}))
                if isinstance(supervisor_progress, Mapping)
                and isinstance(supervisor_progress.get("helper"), Mapping)
                and isinstance(supervisor_progress.get("helper", {}).get("failedCallsByReason"), Mapping)
                else {}
            ),
            "helperCooldownState": (
